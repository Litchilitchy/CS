...
import cs.serving.utils.Conventions
...
object FrontEndApp extends Supportive {
  ...
  val name = "analytics zoo web cs.serving frontend"

  implicit val system = ActorSystem("zoo-cs.serving-frontend-system")
  ...
  val overallRequestTimer = metrics.timer("zoo.cs.serving.request.overall")
  val predictRequestTimer = metrics.timer("zoo.cs.serving.request.predict")
  val putRedisTimer = metrics.timer("zoo.cs.serving.redis.put")
  val getRedisTimer = metrics.timer("zoo.cs.serving.redis.get")
  val waitRedisTimer = metrics.timer("zoo.cs.serving.redis.wait")
  val metricsRequestTimer = metrics.timer("zoo.cs.serving.request.metrics")
