...
import cs.module.AbstractModule
...
trait InferShape {
  ...
  private var _outputShapeValue: Shape = null

  private def inputShapeValue: Shape = _inputShapeValue

  private def outputShapeValue: Shape = _outputShapeValue

  // scalastyle:off
  private def inputShapeValue_=(value: Shape): Unit = {
    _inputShapeValue = value
  }

  private def outputShapeValue_=(value: Shape): Unit = {
  ...
  final def getOutputShape(): Shape = {
    ...
    require(this.isBuilt(), "This cs.module hasn't been built.")
    outputShapeValue
  }
  ...
  def isBuilt(): Boolean = outputShapeValue != null

  def isKerasStyle(): Boolean = false

  def allowRebuilt(): Boolean = false

  /**
   * We suppose the first dim is batch
   */
  private def computeOutputShape(inputShape: Shape): Shape = {
