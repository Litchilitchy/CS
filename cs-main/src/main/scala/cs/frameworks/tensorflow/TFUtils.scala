...
import cs.module.{AbstractCriterion, AbstractModule, Activity}
...
object TFUtils {
  ...
  private def getTrainMeta(trainMetaPath: Path) = {
  ...
  def tf2bigdl(t: TTensor[_], output: Tensor[_]): Unit = {
  ...
  private def byte2float(src: Array[Byte], dest: Array[Float], offset: Int): Unit = {
  ...
  private def int2float(src: Array[Int], dest: Array[Float], offset: Int): Unit = {
    val length = src.length
    var i = 0
    while (i < length) {
      dest(offset + i) = src(i).toFloat
  ...
  private def long2float(src: Array[Long], dest: Array[Float], offset: Int): Unit = {
    val length = src.length
    var i = 0
    while (i < length) {
      dest(offset + i) = src(i).toFloat
  ...
  private def double2float(src: Array[Double], dest: Array[Float], offset: Int): Unit = {
    val length = src.length
    var i = 0
    while (i < length) {
      dest(offset + i) = src(i).toFloat
